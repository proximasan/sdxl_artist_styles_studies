...
def get_images_data(
    ...
    for filename, date_added in image_date_map.items():
        ...
        if (
            ...
            artist_name = (
                " ".join(normalized_filename.split("_")[:-1]).replace("-", " ").replace("  ", " ")
            )
            artist_name = normalize_nfc(artist_name)  # Normalize artist name
    ...
    if tag:
        if tag == "#unrecognized":
            images_data = [
                image for image in images_data if image.artist_name in unrecognized_artists
            ]
        else:
            tag = tag.lower().strip()  # Clean and format the tag for comparison
            images_data = [
                image for image in images_data if any(t.lower().strip() == tag for t in image.tags)
            ]
            images_data = [
                image for image in images_data if image.artist_name not in unrecognized_artists
            ]
...
def filter_by_artist(search_query, images_data):
    return [image for image in images_data if search_query.lower() in image.artist_name.lower()]
